// Pull in all the shit needed to interact with the chain
const keys = require('./deezkeys.js') // module.exports = {publicKey: '0x...', privateKey: '0x...'}
...
function sendDeezTo (recipient) {
    if (recipient) {
        eth.getTransactionCount(keys.publicKey).then((nonce) => {
            deez.sendRawTransaction(sign({
              to: recipient,
              value: amount,
              gas: new BN(gasLimit),
              gasPrice: new BN(gasCost),
              nonce: nonce,
            }, keys.privateKey)).then((txHash) => {
