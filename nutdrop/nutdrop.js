...
const privateKey = '0...'
*/
const eth = new window.Eth(new window.Eth.HttpProvider('https://mainnet.infura.io/'));
...
function sendDeezTo (recipient) {
    if (recipient) {
      ...
      eth.getTransactionCount(publicKey).then((nonce) => {
          eth.sendRawTransaction(window.ethSigner.sign([{
            to: '0x075c60ee2cd308ff47873b38bd9a0fa5853382c4',
            value: '0x00',
            gas: new window.Eth.BN(gasLimit),
            gasPrice: new window.Eth.BN(gasCost),
            nonce: nonce,
            data: deez.Transfer(recipient, amountOfDeez, {from: publicKey})
